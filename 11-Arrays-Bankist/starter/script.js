...
const checkDogs = function (arr, arr2) {
  const dogsJuliaCopy = arr.slice(1, -2);
  const fullarr = [...dogsJuliaCopy, ...arr2]
  fullarr.forEach(function (age, posicao, arr){
    const oldOrPuppy = age > 2 ? `is an adult, and is ${age} years old`: `is still a puppy 🐶`;
    console.log(`Dog number ${posicao + 1} ${oldOrPuppy}`);
  })
};
...
// const balance = movements.reduce(funtion (acc, cur, i, arr) {
...
const calcAverageHumanAge = function (ages) {
  ...
  const dogsAtLest18 = humanAges.filter(age => age > 17);
  console.log(dogsAtLest18);
  const avg =
    dogsAtLest18.reduce((acc, cur) => acc + cur, 0) / dogsAtLest18.length;
...
//1.
const huskysWeight = breeds.find(breed => breed.breed === 'Husky').averageWeight;
  // .filter(x => x.breed === 'Husky')
  // .reduce((acc, cur, _, arr) => acc + cur.averageWeight / arr.length, 0);
console.log(huskysWeight);

//2.
const dogBothActivities = breeds.find( breed => breed.activities.includes('fetch') && breed.activities.includes('running')).breed;
  // .filter(act => act.activities.some(x => x === 'running'))
  // .filter(act => act.activities.some(x => x === 'fetch'));
...
//   ...new Set(
//     breeds
//       .filter(x => x.activities.some(x => x === 'swimming'))
...
//7
console.log(breeds.some(act => act.activities.length > 2));
...
console.log(
  breeds
    .filter(x => x.activities.some(x => x === 'fetch'))
    .find(dog => Math.max(dog.averageWeight)).averageWeight
);
...
const sarahDogEating = `Sarah's dog is eating ${
...
const { ownersTooLittle, ownersTooMuch } = dogs.reduce(
  (acc, cur) => {
    // acc[
    //   cur.curFood < cur.recFood * 0.9
    //     ? 'ownersTooLittle'
    //     : 'ownersTooMuch'
    // ].push(...cur.owners);

    if (cur.curFood < cur.recFood * 0.9) {
      acc.ownersTooLittle.push(...cur.owners);
    } else if (cur.curFood > cur.recFood * 1.1) {
      acc.ownersTooMuch.push(...cur.owners);
    }
...
// 10.
const dogsSortedbyRecfoog = dogs.toSorted((a, b) => a.recFood - b.recFood);
console.log(dogsSortedbyRecfoog);
